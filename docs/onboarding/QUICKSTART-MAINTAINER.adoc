...
// Replace rsr-template-repo, systemet, {{DEPS}} with actuals
...
| `$PREFIX/share/systemet/`
| Data files, assets

| `$PREFIX/share/doc/systemet/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/systemet/config.toml`

Fallback: `$HOME/.config/systemet/config.toml`
...
Or via OPSM: `opsm update systemet`
...
[source,bash]
----
just install --prefix=/opt/systemet-instance1 --config=/etc/systemet/instance1.toml
just install --prefix=/opt/systemet-instance2 --config=/etc/systemet/instance2.toml
----
